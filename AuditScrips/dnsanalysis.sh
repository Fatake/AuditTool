...
for d in $(cat pentest/targets/domains.txt); do
    amass enum -brute -w /opt/rlyeh/dictionaries/subdomain.all.txt -d $d >> pentest/foundsubdomains.txt
    ...
    #amass enum -brute -w /opt/rlyeh/dictionaries/subdomain.plusFinancial.plusSpanish.7k.lst -d $d >> pentest/foundsubdomains.txt
done 
'
