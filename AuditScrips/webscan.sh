...
SECLIST_PATH="/home/kali/Documents/Tools/SecLists/"
EXTENSIONS="conf,config,bak,backup,swp,old,db,sql,asp,aspx,aspx~,asp~,py,py~,rb,rb~,php,php~,bak,bkp,cache,cgi,conf,csv,html,inc,jar,js,json,jsp,jsp~,lock,log,rar,old,sql,sql.gz,sql.zip,sql.tar.gz,sql~,swp,swp~,tar,tar.bz2,tar.gz,txt,wadl,zip,.log,.xml,.js.,.json"
RECURSION="-recursion -recursion-depth 2"
REPLAY_PROXY="-replay-proxy http://127.0.0.1:8080"
THREADS="-t 10"

#Optional
COOKIES="-D '\"visid_incap_1358118=aV3YZIcXTGqCqpFEm5u2JFlPK2MAAAAAQUIPAAAAAAB7dZiNVP+1CefacooTDh4N; JSESSIONID=L7k4jrnpsxhLn0Wf24GngjG8SLvPgJF1MBbRTn481mfpdPDQjP0G!-2056007853; incap_ses_1054_1358118=NMx4cBE6YQwj3p8nfZCgDtRTK2MAAAAAh3JZUrGB7Bk1mSoHEOccJg==; incap_ses_1059_1358118=V0+hZc5RrVfW0BaOBlOyDjNUK2MAAAAA54wBHz4W4nWZva3uAUdUXQ==; incap_ses_1435_1358118=y66nYdwpcjUmiUYyUybqE+tUK2MAAAAAcSZUgFsiiv0foqvSHjvDDA==; incap_ses_1053_1358118=L3kjCb0anFK6yGNWGgKdDtltK2MAAAAA+OLBBGm0QQg0owIQleLn0A==\"'"

i=1
echo -e "[i] Auto FFUF"
echo -e "<------------------------------->";
for target in $(cat Pentest_${NAME}/targets/domains.txt); do
	COMMAND="ffuf -r ${RECURSION} ${REPLAY_PROXY} -w ${SECLIST_PATH}/Discovery/Web-Content/big.txt -e ${EXTENSIONS} -ac -o ffuf_target${i}.html -of html ${THREADS} -u ${target}/FUZZ"
